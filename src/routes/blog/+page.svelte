...
<main style="background-color: #f5f5f5" bind:clientWidth={width} bind:clientHeight={height}>
	<Seo title="Blog" titleTemplate="%t% | LuterGS" />
	...
	<Paper>
		<Group position="center">
			<Text
				...
				variant="gradient"
				weight="bold"
				gradient={{ from: 'dark', to: 'cyan', deg: 45 }}>글 목록</Text
			>
		</Group>
	</Paper>
	<Space h="md" />
	...
	<AnimatedNotification visible={isUserNotLogined} icon={CrossCircled} --top="1.5rem">
		...
		<Paper override={{ padding: '6px 6px 6px 6px' }}>
			<Group position="apart">
				<ClickablePaper
					...
					padding={'13px 10px 13px 10px'}>◀️</ClickablePaper
				>
				<ClickablePaper
					onClick={() => {
						moveNextPage();
