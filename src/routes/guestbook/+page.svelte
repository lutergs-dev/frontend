...
<main style="background-color: #f5f5f5" bind:clientWidth={width} bind:clientHeight={height}>
	<Seo title="Guestbook" titleTemplate="%t% | LuterGS" />
	...
	<Paper>
		<Group position="center">
			<Text
				...
				variant="gradient"
				weight="bold"
				gradient={{ from: 'dark', to: 'cyan', deg: 45 }}>방명록</Text
			>
		</Group>
	</Paper>
	<Space h="md" />
			...
			{#each data.comments as comment}
				<Paper>
					<Group position="apart">
						<Text variant="gradient" weight="bold" gradient={{ from: 'dark', to: 'cyan', deg: 45 }}
							>{comment.name}</Text
						>
					...
					<Group position="left">
						{#if comment.deletePressed}
							<TextInput placeholder="비밀번호 입력" bind:value={comment.password} />
							<Button
								compact
								variant="gradient"
								gradient={{ from: 'orange', to: 'dark', deg: 45 }}
								on:click={() => {
	...
	<Paper>
		...
		<Space h="md" />
		<svelte:component this={TinyMceEditor} bind:value={commentValue} />
		<Space h="md" />
		<Group position="left">
			<Button
				variant="gradient"
				gradient={{ from: 'dark', to: 'cyan', deg: 45 }}
				on:click={addComment}>저장</Button
			>
			<TextInput
	...
	<FloatingButton backlink={''}>
		<Paper override={{ padding: '6px 6px 6px 6px' }}>
			<Group position="apart">
				<ClickablePaper onClick={movePreviousPage} padding={'13px 10px 13px 10px'}
					>◀️</ClickablePaper
				>
