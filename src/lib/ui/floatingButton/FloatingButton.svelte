<script lang="ts">
	...
	const mainButtonCss = {
		...
		height: '',
		background:
			'linear-gradient(45deg, var(--svelteui-colors-black) 0%, var(--svelteui-colors-cyan600) 100%)',
		color: 'white',
		borderRadius: '2.5rem',
		textAlign: 'center',
	...
	const switchClick = () => {
		if (isClicked === false) {
			isClicked = true;
			mainButtonCss.background =
				'linear-gradient(45deg, var(--svelteui-colors-black) 0%, var(--svelteui-colors-yellow600) 100%)';
		} else {
			isClicked = false;
			mainButtonCss.background =
				'linear-gradient(45deg, var(--svelteui-colors-black) 0%, var(--svelteui-colors-cyan600) 100%)';
